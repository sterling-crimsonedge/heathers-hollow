...
- **New follow-up: Hugo shared-weather and Clover day-5 trust unlock event hooks.** With the basic `tuning` knobs shipped, the two remaining doc-level Hugo/Clover calibration items both need an event hook rather than a static JSON field. Hugo's "+1 affection on shared-weather lines" needs greeting-detection plus a world weather/time match. Clover's "trust cap halved for the first 5 in-game days, then unlocks" needs a per-relationship day-of-first-meeting field plus a date-aware override layer above `_effective_talk_caps`. Neither blocks the morning demo. If a future heartbeat picks them up, the schema bump should land as new optional `tuning` keys (e.g. `shared_weather_affection_bonus` and `trust_cap_unlocks_on_day`) so the rubric stays in JSON.
