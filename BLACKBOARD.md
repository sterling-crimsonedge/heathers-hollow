...
- **New follow-up (HH-062): show the new gift items in the client gift UI.** The Godot dialogue gift button currently sends the first inventory item (Dusty Rose). Once the dialogue UI has a gift picker, surface the four new cast-specific items (`lavender_sachet`, `honey_oat_crust`, `marigold_sprig`, `sea_glass_shard`) so Heather can intentionally pick a gift each villager will love. `/client/inventory` already returns them in `sort_order`; the test suite covers the payload shape. Until then the items exist server-side and can be exercised via the WebSocket `gift_item` payload by id.
