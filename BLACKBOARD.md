...
- HH-062 per-villager loved-gift rubric (eighth Cowork heartbeat): `server/ai/personality.py` grew an optional `loved_tags: list[str]` field on `Personality`, normalized to lowercase non-empty strings at load time and absent from `public_villager_summary` so clients can't mine the precise rubric from `/client/bootstrap` or `/villagers/{id}`. `server/ai/conversation.py` added a module-level `DEFAULT_LOVED_TAGS` frozenset (the legacy `{flower, porcelain, tea, handmade, garden vegetables}` set) and replaced the hardcoded global check inside `_gift_preference` with a per-villager `_loved_tags_for(personality)` lookup that prefers the personality's explicit `loved_tags` and falls back to `DEFAULT_LOVED_TAGS` for any older config — so the four canonical MVP villagers now score "loved" against four distinct rubrics. Margot keeps `[flower, porcelain, tea, handmade, garden, soft_color]`, Fern gets `[tea, herb, lavender, chamomile, handmade, rain]`, Hugo gets `[bread, tea, rain, sea, wood, well-made, warm]`, and Clover gets `[shiny, broken, marigold, orange, trinket, smooth, keepsake, stone]`. The category string is now part of the intersection check so Clover loves a `category="trinket"` smooth pebble without needing "trinket" in their JSON likes list. `server/world/inventory.py` added four cast-specific starter gifts at `sort_order >= 50` so the demo can show four distinct delighted reactions: `lavender_sachet` (Fern loved), `honey_oat_crust` (Hugo loved), `marigold_sprig` (Clover + Margot loved), and `sea_glass_shard` (Clover loved, with a "sea" tag that also brushes Hugo). `server/tests/test_gift_relationship.py` grew per-villager loved-cases asserting `mood == "delighted"`, `metadata.preference == "loved"`, `affection >= 5`, and `trust >= 2` on each, plus an explicit Hugo + Dusty Rose neutral assertion (`mood == "shy"`) proving that gifts now read as personality rather than a single global rubric. `server/tests/test_personality_configs.py` validates the loved_tags schema (non-empty lowercase strings) and enforces signature tags for the canonical cast (Margot has `porcelain`+`flower`, Fern has `lavender`+`herb`, Hugo has `bread`+`sea`, Clover has `marigold`+`shiny`). `docs/AI_ARCHITECTURE.md`'s HH-006 status block now records HH-062 as shipped.
...
- HH-006 per-villager `tuning` JSON calibration (eleventh heartbeat — Codex 2026-05-15 overnight scheduled task): `server/ai/personality.py` adds an optional `tuning: dict[str, Any]` field on `Personality` plus a new `Personality._coerce_tuning(raw)` helper that coerces the supported keys (`affection_per_talk_cap`, `trust_per_talk_cap`, `negative_talk_per_day_cap`, `loved_gift_mood_lock_hours`, `first_gift_bonus_tier`) to their expected types at load time and drops anything else (so the block can't grow a backdoor for client-facing data). `server/ai/conversation.py` adds three new helpers — `_effective_talk_caps(personality)` returns `(affection_cap, trust_cap, negative_cap)` with per-villager overrides folded in, `_effective_loved_pin_minutes(personality)` converts `loved_gift_mood_lock_hours` to monotonic in-game minutes for `MoodTracker.pin`, and `_first_gift_bonus_enabled(personality)` honors a `first_gift_bonus_tier=0` opt-out — and wires them into `_apply_talk_caps` (now takes an optional `personality`) and `handle_gift` (uses the override pin duration for loved gifts, gates the HH-006 first-of-kind tier bump on the opt-out flag). The canonical MVP cast JSONs now ship tuning blocks: Margot (`affection_per_talk_cap=1`, `trust_per_talk_cap=0`, `loved_gift_mood_lock_hours=3` — fragile, slow-warming, loved-gifts-feel-biggest), Hugo (`affection_per_talk_cap=1`, `loved_gift_mood_lock_hours=3` — gruff baker who lingers on praise), Clover (`trust_per_talk_cap=0`, `loved_gift_mood_lock_hours=1.5` — testing whether Heather keeps promises, moves on quickly), and Fern (no tuning block — her "rewards-consistency-over-big-gestures" register is exactly the global defaults). `tuning` is intentionally absent from `public_villager_summary` so clients can't read the rubric. New coverage: `server/tests/test_tuning_calibration.py` (six runners — Margot slow-warming, Hugo gruff-talk, Clover testing-trust, Fern default caps, loved-gift pin duration override for Margot/Hugo/Clover, and a `first_gift_bonus_tier=0` opt-out exercise via an in-memory PersonalityStore patch). `server/tests/test_personality_configs.py` validates the tuning schema (allowed keys, non-negative numbers, `loved_gift_mood_lock_hours > 0`) and locks in the canonical-cast calibration values so accidental JSON edits surface as test failures. `server/tests/test_api_contract.py` adds bootstrap-leak guardrails asserting that no villager payload exposes `tuning` or `loved_tags`. The `test_talk_caps.py` test villager was rerouted from Margot (now overrides the caps) to Fern (keeps defaults) so it still exercises the global cap-mechanism path. `test_conversation_relationship.py` was updated to assert Margot's seeded heather (8, 12, 2) lands at (9, 12, 3) after one personal-disclosure turn — affection +1 (Margot's tightened cap), trust +0 (Margot's slow-trust cap), familiarity +1 (uncapped). `docs/AI_ARCHITECTURE.md`'s HH-006 status block and the "Per-villager calibration" section now record the field shape, supported keys, canonical-cast table, and validation pointers.
- HH-062 Godot gift picker (fourteenth Cowork heartbeat — 2026-05-15 overnight scheduled task): `game/scripts/main.gd` now opens a dialogue-side per-item gift picker instead of always sending the default Dusty Rose. A new `gift_picker_panel: PanelContainer` + `gift_picker_list: VBoxContainer` is built inside `_build_ui()` and slotted into the dialogue stack directly above the input row via `stack.move_child(...)`. `gift_button.text` flips from `"Gift Rose"` to `"Gift..."`, and `_on_gift_pressed()` now calls `_open_gift_picker()` which rebuilds the list from the cached `starter_inventory` (falling back to `[_starter_gift_payload()]` so the picker is never empty before `/client/bootstrap` resolves). Each picker button is labeled `"display_name — category"` with the server's `gift_prompt` as a hover tooltip, paralleling the twelfth-heartbeat web demo. Pressing a picker button calls `_on_gift_picker_item_pressed(item)` -> `_send_gift_item(item)`, which routes through the same `_interaction_context(villager, {"gift_source": "starter_inventory"})` payload as before — only the gift dict changes. The keyboard `gift` shortcut still defaults to the Dusty Rose via `_send_gift_item(_starter_gift_payload())`. `_close_dialogue()` calls `_close_gift_picker()`. `_apply_bootstrap_inventory()` updates the gift-button tooltip to "Choose a gift from the starter inventory (N items)." instead of advertising a specific item. Coverage: new `test_main_gift_picker_wiring()` in `game/tests/test_godot_project_static.py` locks in the picker scaffolding, every per-item helper, the `_send_gift_item`/`_send_starter_gift` split, the picker-button signal binding, the `_close_dialogue` -> `_close_gift_picker` lifecycle, and that the picker body never references private personality fields. With this heartbeat, Heather can pick Lavender Sachet for Fern, Honey Oat Crust for Hugo, Marigold Sprig or Sea Glass Shard for Clover, and Dusty Rose for Margot directly from the Godot panel — bringing the Godot client to parity with the web demo on the HH-062 per-villager loved-gift rubric.
- HH-006 mood pin for loved gifts (ninth Cowork heartbeat): `server/ai/mood.py` now persists optional `pinned_mood: str | None` and `pinned_until_minute: int | None` fields inside `mood_state` (under the villager's self-relationship metadata). A new `MoodTracker.pin(villager_id, mood, *, world, duration_minutes=DEFAULT_PIN_DURATION_MINUTES)` records the pin against a monotonic in-game-minute count `(day - 1) * 1440 + minute_of_day` computed from the world snapshot, gives the pinned mood a +1.0 initial score weight (so the lingering warmth survives pin expiry), and writes the pinned mood into `mood_state.current` so `/villagers/{id}` and `/client/villagers/{id}/context` surface the loved-gift reading without exposing the rubric. `tick()` is now pin-aware: while the pin is active it adds a `PINNED_MOOD_TICK_REINFORCEMENT=0.45` weight on every tick (so score-resolved dominant doesn't fight the pin) and reports the pinned mood regardless of baseline; when the deadline elapses it clears the pin metadata and lets the baseline reassert. `nudge()` preserves an existing pin's metadata and overrides the resolved current to the pinned mood so a conversation turn during the pin window doesn't wash it out. `current_mood(..., world=...)` returns the pinned mood when world context is provided and the pin is live, falling back to the persisted `current` when world is absent so existing callers (including `api/server.py:public_relationship_metadata`) keep working unchanged. `handle_gift` in `server/ai/conversation.py` now calls `mood_tracker.pin(villager_id, tracker_mood, world=self.world_state)` only when `preference == "loved"` — liked/neutral/disliked gifts intentionally don't pin, matching the cozy "loved gifts feel biggest, missteps are a small note" rubric. New coverage in `server/tests/test_mood.py:run_mood_pin_check()` pins `excited` on Margot at day-1 10:00 morning, runs several baseline ticks inside the pin window (asserting current stays `excited` and `current_mood(..., world=...)` agrees), then ticks again past the `DEFAULT_PIN_DURATION_MINUTES=120` deadline and asserts pin metadata is cleared and a fresh `MoodTracker` instance on the same DB sees the expired state. `server/tests/test_gift_relationship.py` grew per-villager loved-gift pin assertions for Fern/Hugo/Clover (`pinned_mood == "excited"`, positive `pinned_until_minute`, `mood_state.current == "excited"`) and a guard that the disliked gift path doesn't introduce its own pin. `docs/AI_ARCHITECTURE.md`'s HH-006 status block records the pin as shipped and the "Mood pinning for loved gifts" recommendation is now marked shipped in the ninth Cowork heartbeat. `public_relationship_metadata` keeps its existing allowlist so `pinned_mood` and `pinned_until_minute` remain server-private — only the resolved `mood_state.current` is publicly exposed as `current_mood`.
...
- HH-062 per-villager loved-gift focused validation (2026-05-15 overnight, eighth Cowork heartbeat): the extended `server/tests/test_gift_relationship.py` (Fern/Hugo/Clover loved + Hugo-on-Dusty-Rose neutral) and `server/tests/test_personality_configs.py` (loved_tags schema + canonical-cast signature tags), plus the full 14-test matrix, run as `uv run --python 3.12 --with-requirements server/requirements.txt python -m server.tests.{test_memory_roundtrip,test_personality_configs,test_api_contract,test_mood,test_world_state,test_gift_relationship,test_conversation_relationship,test_social_memory_conversation,test_mobile_notifications,test_away_interactions,test_demo_seed,test_demo_storyline,test_ollama_provider,test_talk_caps}`. Also ran `uv run --python 3.12 --with-requirements server/requirements.txt python -m server.tests.test_live_demo_stack`, `uv run --python 3.12 --with-requirements server/requirements.txt python -m server.tests.test_websocket_roundtrip --start-server`, `python3 game/tests/test_godot_project_static.py`, `python3 game/tests/test_web_demo_static.py`, `node --check game/web/{main.js,scene.js}`, `uv run --python 3.12 --with-requirements server/requirements.txt python -m compileall server`, and `git -C ... diff --check`. All green.
- HH-006 per-villager tuning focused validation (2026-05-15 overnight, eleventh Codex heartbeat): the new `server/tests/test_tuning_calibration.py` plus the extended `server/tests/test_personality_configs.py`, `server/tests/test_talk_caps.py` (rerouted to Fern), and `server/tests/test_conversation_relationship.py`, then the full 15-test matrix `uv run --python 3.12 --with-requirements server/requirements.txt python -m server.tests.{test_memory_roundtrip,test_personality_configs,test_api_contract,test_mood,test_world_state,test_gift_relationship,test_conversation_relationship,test_social_memory_conversation,test_mobile_notifications,test_away_interactions,test_demo_seed,test_demo_storyline,test_ollama_provider,test_talk_caps,test_tuning_calibration}` (all green), the live demo stack `... test_live_demo_stack` and websocket roundtrip `... test_websocket_roundtrip --start-server` (both green — Margot's Dusty Rose reply still lands as `delighted`), `python3 game/tests/test_godot_project_static.py`, `python3 game/tests/test_web_demo_static.py`, `node --check game/web/{main.js,scene.js}`, `uv run --python 3.12 --with-requirements server/requirements.txt python -m compileall server`, and `git diff --check` clean.
- HH-062 Godot gift picker focused validation (2026-05-15 overnight, fourteenth Cowork heartbeat): the new `test_main_gift_picker_wiring()` runner in `game/tests/test_godot_project_static.py`, plus the full 15-test matrix `uv run --python 3.12 --with-requirements server/requirements.txt python -m server.tests.{test_memory_roundtrip,test_personality_configs,test_api_contract,test_mood,test_world_state,test_gift_relationship,test_conversation_relationship,test_social_memory_conversation,test_mobile_notifications,test_away_interactions,test_demo_seed,test_demo_storyline,test_ollama_provider,test_talk_caps,test_tuning_calibration}`, `... test_live_demo_stack`, `... test_websocket_roundtrip --start-server` (Margot's "Oh, Dusty Rose..." reply still lands as `delighted`), `python3 game/tests/test_godot_project_static.py`, `python3 game/tests/test_web_demo_static.py`, `node --check game/web/{main.js,scene.js}`, `python3 -m compileall server`, `git diff --check`. All green.
- HH-006 mood-pin focused validation (2026-05-15 overnight, ninth Cowork heartbeat): the new `run_mood_pin_check()` case in `server/tests/test_mood.py` plus the per-villager loved-gift pin assertions in `server/tests/test_gift_relationship.py`, then the full 14-test matrix `uv run --python 3.12 --with-requirements server/requirements.txt python -m server.tests.{test_memory_roundtrip,test_personality_configs,test_api_contract,test_mood,test_world_state,test_gift_relationship,test_conversation_relationship,test_social_memory_conversation,test_mobile_notifications,test_away_interactions,test_demo_seed,test_demo_storyline,test_ollama_provider,test_talk_caps}` (all green), the live stack `uv run --python 3.12 --with-requirements server/requirements.txt python -m server.tests.test_live_demo_stack` and `... test_websocket_roundtrip --start-server` (both green and the Margot "delighted" gift reply lands cleanly), `python3 game/tests/test_godot_project_static.py`, `python3 game/tests/test_web_demo_static.py`, `node --check game/web/{main.js,scene.js}`, `uv run --python 3.12 --with-requirements server/requirements.txt python -m compileall server`, and `git diff --check` clean.
...
### HH-062 - Per-Villager Loved-Gift Rubric

**Status:** DONE - Claude/Cowork (2026-05-15 overnight, eighth heartbeat)
**Owner:** Claude/Cowork
**Goal:** Make gift preferences read as personality. The single global `{flower, porcelain, tea, handmade, garden vegetables}` loved set meant Hugo loved a porcelain button just as much as Margot, which flattened the cast for any gift demo.

**Acceptance:**

- Add an optional, backward-compatible `loved_tags` list to `Personality` so each villager has a distinct loved rubric.
- Keep server payloads conservative — `loved_tags` must not appear in `public_villager_summary` or `/client/bootstrap`.
- Provide a `DEFAULT_LOVED_TAGS` fallback so any pre-HH-062 config keeps its old behavior.
- Populate `loved_tags` for the four canonical MVP villagers (Margot/Fern/Hugo/Clover) with at least one signature tag per villager.
- Expand `STARTER_INVENTORY` with one cast-specific item per villager so the demo can show four distinct delighted reactions.
- Extend `test_gift_relationship.py` with Fern/Hugo/Clover loved-gift assertions and an explicit "Hugo does not love Dusty Rose" neutral assertion.
- Extend `test_personality_configs.py` with a loved_tags schema check and signature-tag enforcement.
- Update `docs/AI_ARCHITECTURE.md` HH-006 status block to record the change.

**Resolution:** Shipped exactly that. `server/ai/personality.py` adds `loved_tags: list[str] = []` defaulting to empty and lowercased at load time. `server/ai/conversation.py` adds `DEFAULT_LOVED_TAGS` and a new `_loved_tags_for(personality)` helper; `_gift_preference` now intersects the gift's `tags + category` against the per-villager rubric (or the legacy default) instead of a single hardcoded set. Margot keeps the original five-tag rubric plus `garden`/`soft_color`; Fern's signature tags are `lavender`/`herb`; Hugo's are `bread`/`sea`; Clover's are `marigold`/`shiny` plus `trinket`/`smooth`/`stone` so a Smooth Pebble lands as a loved gift for them. New starter items: `lavender_sachet`, `honey_oat_crust`, `marigold_sprig`, `sea_glass_shard`. Coverage: per-villager loved cases in `test_gift_relationship.py` + a Hugo-on-Dusty-Rose neutral case, plus the schema/signature checks in `test_personality_configs.py`. Validation: the full 13-server-test matrix, the live demo stack and websocket roundtrip, both Godot/web static tests, `node --check` for the web demo JS, and `python3 -m compileall server` all green.
...
- **2026-05-15 - Cowork (overnight, fourteenth — scheduled-task pass):** Cleared the **Godot half of the HH-062 gift picker follow-up** that the twelfth Codex heartbeat had left open. Before this heartbeat, `game/scripts/main.gd` `_on_gift_pressed()` always called `_send_starter_gift()`, which sent the cached Dusty Rose payload via `_starter_gift_payload()` — even though `/client/bootstrap` was already populating `starter_inventory` with all eight HH-062 starter items. The web demo's twelfth-heartbeat picker is now mirrored in the Godot dialogue panel, while the keyboard `gift` shortcut keeps the muscle-memory Dusty Rose quick-send. Five `main.gd` changes wire it up: (1) **Picker scaffolding.** Two new members `gift_picker_panel: PanelContainer` and `gift_picker_list: VBoxContainer`, built inside `_build_ui()` and placed in the existing dialogue `stack` directly above the input row via `stack.move_child(gift_picker_panel, stack.get_child_count() - 2)`. The picker panel has its own soft `_gift_picker_panel_style()` StyleBoxFlat (cream/blush border, 1px) so it reads as a sub-popup rather than a second window. Header is "Choose a gift" + a Cancel button wired to `_close_gift_picker`. (2) **Gift button is now the picker entry.** `gift_button.text` changes from `"Gift Rose"` to `"Gift..."` and its tooltip drops the Dusty-Rose hardcoding for `"Choose a gift from the starter inventory."`. `_on_gift_pressed()` now calls `_open_gift_picker()` instead of `_send_starter_gift()`. (3) **Per-item helpers.** `_gift_display_name()`, `_gift_caption()`, and `_gift_tooltip()` parallel the web demo's `renderGiftRow()` shape — name span, category caption, and a hover tooltip / aria-label sourced from `gift_prompt`. `_gift_picker_items()` returns the cached `starter_inventory` when populated, falling back to `[_starter_gift_payload()]` so the picker is never empty even before `/client/bootstrap` resolves. (4) **Per-item send.** A new `_send_gift_item(item: Dictionary)` runs the existing send flow (open dialogue if needed, append "Heather gives X a Y." line, set status, call `conversation_client.send_gift(villager.villager_id, PLAYER_ID, gift_payload, context)`), but with a per-item payload instead of the always-Dusty-Rose `_starter_gift_payload()`. `_normalize_gift_item(item)` defensively copies the dict and falls back to the default if a stale picker entry lacks `item_id`. `_send_starter_gift()` is now a thin wrapper around `_send_gift_item(_starter_gift_payload())` so the keyboard shortcut behavior is preserved. The shared `_interaction_context(villager, {"gift_source": "starter_inventory"})` call still drives both paths so the server-side gift event metadata is identical. (5) **Lifecycle.** `_close_dialogue()` calls `_close_gift_picker()` so a stale list can't outlive the dialogue. `_apply_bootstrap_inventory()` updates the gift-button tooltip to "Choose a gift from the starter inventory (N items)." so Heather sees the picker is populated, instead of advertising a single item. **Coverage.** New `test_main_gift_picker_wiring()` runner in `game/tests/test_godot_project_static.py` asserts the picker scaffolding, the gift-button entry-point behavior (including the negative assertion that the old `"Gift Rose"` text is gone), the per-item helpers and their `gift_prompt`/`category`/`display_name` reads, `_send_gift_item`/`_send_starter_gift` shape, the picker-button binding via `connect(...).bind(bound_item)`, that `_close_dialogue` also closes the picker, and that the picker body never references private personality fields. **Validation matrix (all green):** `python3 game/tests/test_godot_project_static.py`, `python3 game/tests/test_web_demo_static.py`, `node --check game/web/{main.js,scene.js}`, `python3 -m compileall server`, plus the full 15-test `uv run --python 3.12 --with-requirements server/requirements.txt python -m server.tests.{test_memory_roundtrip,test_personality_configs,test_api_contract,test_mood,test_world_state,test_gift_relationship,test_conversation_relationship,test_social_memory_conversation,test_mobile_notifications,test_away_interactions,test_demo_seed,test_demo_storyline,test_ollama_provider,test_talk_caps,test_tuning_calibration}` matrix, the live stack `... test_live_demo_stack` and `... test_websocket_roundtrip --start-server` (both green; Margot's "Oh, Dusty Rose..." reply still lands as `delighted`), and `git diff --check` clean. The stale `.git/index.lock` re-appeared mid-heartbeat as in earlier overnight runs; cleared via the Desktop Commander host shell per the documented protocol so the commit/push could proceed. **Demo impact.** When `/client/bootstrap` succeeds, Heather can now press the dialogue Gift button to pick Dusty Rose, Chamomile Bundle, Porcelain Button, Smooth Pebble, Lavender Sachet, Honey Oat Crust, Marigold Sprig, or Sea Glass Shard — and the eighth-heartbeat per-villager `loved_tags` rubric means those choices read as four distinct delighted reactions across the canonical MVP cast. Godot runtime validation remains blocked until a `godot4` binary is available, so the new static guardrail is the durable proof.
- **2026-05-15 - Codex (overnight, thirteenth — scheduled-task pass):** Cleared the last unstruck HH-006 doc-level follow-up — Hugo shared-weather and Clover day-5 trust unlock event hooks, both as new optional `tuning` keys. `Personality._coerce_tuning` accepts `trust_cap_unlocks_on_day` (int > 0) and `shared_weather_affection_bonus` (int ≥ 0). `_effective_talk_caps` reads `first_seen_day` off the relationship row and drops a `trust_per_talk_cap` override once `world_day >= first_seen_day + N`. New `_shared_weather_bonus()` helper in `server/ai/conversation.py` matches the player's text against a small weather/time vocabulary (rain/storm/snow/sun/cloud + literal time-of-day labels) and the world's `weather` substring / `time_label`; on a match it returns the personality's `shared_weather_affection_bonus` int and stamps `last_shared_weather_day` + `last_shared_weather_marker` so it cannot refire the same in-game day. The bonus stacks past the daily affection cap so the moment lands. `handle_player_message` and `handle_gift` both stamp `first_seen_day` on the very first interaction and never overwrite it. **Per-villager wiring:** Clover gains `trust_cap_unlocks_on_day=5`; Hugo gains `shared_weather_affection_bonus=1`. **Coverage:** five new runners in `server/tests/test_tuning_calibration.py` (`run_clover_trust_unlock_check`, `run_clover_trust_unlock_locked_window_check`, `run_hugo_shared_weather_bonus_check`, `run_hugo_shared_evening_bonus_check`, `run_fern_no_shared_weather_bonus_check`); extended `test_personality_configs.py` schema check + canonical-cast lock-in for the two new keys; existing `test_api_contract.py` tuning-leak guardrail still applies (allowlist unchanged). **Docs:** updated `docs/AI_ARCHITECTURE.md` HH-006 status block, supported-keys list, per-villager bullets, and the canonical-cast calibration table. **Validation matrix (all green):** `uv run --python 3.12 --with-requirements server/requirements.txt python -m server.tests.{test_memory_roundtrip,test_personality_configs,test_api_contract,test_mood,test_world_state,test_gift_relationship,test_conversation_relationship,test_social_memory_conversation,test_mobile_notifications,test_away_interactions,test_demo_seed,test_demo_storyline,test_ollama_provider,test_talk_caps,test_tuning_calibration,test_live_demo_stack}` + `... test_websocket_roundtrip --start-server` (Margot's "Oh, Dusty Rose..." reply still lands as `delighted`) + `python3 game/tests/test_godot_project_static.py` + `python3 game/tests/test_web_demo_static.py` + `node --check game/web/{main.js,scene.js}` + `uv run --python 3.12 --with-requirements server/requirements.txt python -m compileall server` + `git diff --check` clean.
- **2026-05-15 - Cowork (overnight, ninth):** Shipped the HH-006 mood-pin schema. `server/ai/mood.py` adds optional `pinned_mood: str | None` and `pinned_until_minute: int | None` fields on `mood_state` (persisted under the villager's self-relationship metadata), a new `MoodTracker.pin(villager_id, mood, *, world, duration_minutes=DEFAULT_PIN_DURATION_MINUTES)` method that records the pin against a monotonic `(day - 1) * 1440 + minute_of_day` count computed from the world snapshot, pin-aware `tick()` (reinforces the pinned mood at weight `PINNED_MOOD_TICK_REINFORCEMENT=0.45` while live, clears metadata on expiry), pin-aware `nudge()` (preserves the pin and overrides the resolved current so a follow-up conversation can't wash it out), and a `current_mood(..., world=...)` keyword arg that honors the pin when world context is available. `handle_gift` in `server/ai/conversation.py` calls `mood_tracker.pin(villager_id, tracker_mood, world=self.world_state)` only when `preference == "loved"` — liked/neutral/disliked gifts intentionally don't pin. New coverage: `run_mood_pin_check()` in `server/tests/test_mood.py` (pins Margot's `excited` at day-1 10:00 morning, asserts current stays `excited` through several ticks inside the 120-minute window, then asserts metadata clears past the deadline with the fresh `MoodTracker` instance on the same DB seeing the expired state) plus per-villager pin assertions in `server/tests/test_gift_relationship.py` (Fern/Hugo/Clover each end the loved-gift path with `pinned_mood="excited"`, positive `pinned_until_minute`, and `mood_state.current == "excited"`, and the disliked path doesn't introduce its own pin). `docs/AI_ARCHITECTURE.md`'s HH-006 status block records the pin as shipped. The `pinned_mood`/`pinned_until_minute` fields stay server-private — `public_relationship_metadata` keeps its existing allowlist and only the resolved `mood_state.current` is publicly exposed as `current_mood`, so no API contract change was needed. Validation: `uv run --python 3.12 --with-requirements server/requirements.txt python -m server.tests.{test_mood,test_gift_relationship,test_memory_roundtrip,test_personality_configs,test_api_contract,test_world_state,test_conversation_relationship,test_social_memory_conversation,test_mobile_notifications,test_away_interactions,test_demo_seed,test_demo_storyline,test_ollama_provider,test_talk_caps,test_live_demo_stack}` (15 server tests all green), `uv run --python 3.12 --with-requirements server/requirements.txt python -m server.tests.test_websocket_roundtrip --start-server` (green, Margot's "delighted" Dusty Rose reply lands cleanly), `python3 game/tests/test_godot_project_static.py`, `python3 game/tests/test_web_demo_static.py`, `node --check game/web/{main.js,scene.js}`, `uv run --python 3.12 --with-requirements server/requirements.txt python -m compileall server`, and `git diff --check`.
...
- **2026-05-15 (overnight, tenth heartbeat) - Claude/Cowork:** Shipped the last two HH-006 gift-path follow-ups that prior heartbeats had left on the recommended list — **first-of-kind gift bonus** and **repeated-gift dampening**. Before this heartbeat, every gift to a villager scored identically regardless of whether it was Heather's first interaction with them or her fifth Dusty-Rose in a row; both were sources of "slot-machine warmth" called out in `docs/AI_ARCHITECTURE.md`. The change: `server/ai/conversation.py` adds `GIFT_PREFERENCE_LADDER=("disliked","neutral","liked","loved")`, `GIFT_REPEAT_WINDOW_DAYS=3`, `GIFT_REPEAT_DAMPEN_THRESHOLD=2`, two new helpers `_bump_gift_preference()` and `_coerce_recent_gifts()`, and weaves them into `handle_gift`. The first gift from any (player, villager) pair bumps preference up one tier on the ladder (`disliked→neutral`, `neutral→liked`, `liked→loved`, `loved` stays put — there's no higher rung), and the relationship row records `gift_first_done=True` so the bonus fires exactly once per pair. A persisted `recent_gifts: list[{item_id, day}]` is maintained alongside, pruned to the trailing 3 in-game days at every write so the metadata blob can't grow unbounded. If the current `item_id` has already appeared in the trailing window `GIFT_REPEAT_DAMPEN_THRESHOLD` times (i.e. the current gift is the 3rd same-item gift in 3 days), the affection delta is halved toward zero (`int(delta/2)`). Trust, familiarity, mood, the memory write, the event publish, and the loved-gift `MoodTracker.pin()` from the ninth heartbeat are intentionally **not** dampened — the moment still registers; only the optimal-play stat reward is dulled. Memory and event metadata now record `base_preference`, `first_of_kind_bonus`, `repeat_gift_dampened`, and `repeat_count_in_window` so future debug/UI work can explain *why* a delta was its observed size; none of those internal-tuning keys are in `PUBLIC_EVENT_METADATA_KEYS` or `PUBLIC_MEMORY_METADATA_KEYS`, so they don't leak via `/events/{id}` or `/memories/{id}`. **Coverage:** two new runners in `server/tests/test_gift_relationship.py` — `run_first_of_kind_gift_bonus_check()` (proves neutral→liked first-gift mood and metadata, disliked→neutral first-gift mood and metadata, loved-stays-loved first-gift, the "exactly once per player" invariant via a second same-player gift, and the `gift_first_done` persistence) and `run_repeated_gift_dampening_check()` (proves the 3rd same-item Dusty Rose halves affection from +5 to +2 while mood/trust stay full, and proves switching to a different loved item (Chamomile Bundle) resets the counter and lands the full +5). The existing 5 steady-state Margot gift cases plus the 4 per-villager loved cases now each get a generic `tuning_warmup` gift first via a new `_burn_first_of_kind()` helper, so they exercise the native preference path the assertions were written for; the warmup adds a constant `WARMUP_AFFECTION=2`/`WARMUP_TRUST=2`/`WARMUP_FAMILIARITY=1` baseline that the assertions now reference symbolically instead of using bare integers. **Validation matrix (all green):** the full 14-server-test `uv run --python 3.12 --with-requirements server/requirements.txt python -m server.tests.{test_memory_roundtrip,test_personality_configs,test_api_contract,test_mood,test_world_state,test_gift_relationship,test_conversation_relationship,test_social_memory_conversation,test_mobile_notifications,test_away_interactions,test_demo_seed,test_demo_storyline,test_ollama_provider,test_talk_caps}` plus the live demo stack `... test_live_demo_stack` and websocket roundtrip `... test_websocket_roundtrip --start-server`, plus `python3 game/tests/test_godot_project_static.py`, `python3 game/tests/test_web_demo_static.py`, `node --check game/web/{main.js,scene.js}`, `uv run --python 3.12 --with-requirements server/requirements.txt python -m compileall server`. `docs/AI_ARCHITECTURE.md`'s HH-006 status block and the "Recommended deltas" table now record both rules as shipped. Remaining HH-006 work after this heartbeat: per-villager `tuning` JSON calibration (e.g. Margot's slower-trust personality bumping `affection_per_talk_cap` to 1 instead of 2, Hugo's "shared weather" line earning the +1 affection bonus, Clover's day-5 trust cap unlock). None blocking the morning demo.
- **2026-05-15 (overnight, eighth heartbeat) - Claude/Cowork:** Shipped HH-062 per-villager loved-gift rubric. Before this heartbeat the gift engine had a single hardcoded global "loved" tag set — every villager loved a porcelain button or a chamomile bundle regardless of personality. After the change, each villager scores "loved" against their own JSON-driven `loved_tags` list (Margot: `flower/porcelain/tea/handmade/garden/soft_color`; Fern: `tea/herb/lavender/chamomile/handmade/rain`; Hugo: `bread/tea/rain/sea/wood/well-made/warm`; Clover: `shiny/broken/marigold/orange/trinket/smooth/keepsake/stone`), with a `DEFAULT_LOVED_TAGS` fallback for any legacy config. Four new starter inventory items mean the morning demo can land four distinct "delighted" beats — Lavender Sachet for Fern, Honey Oat Crust for Hugo, Marigold Sprig for Clover and Margot (it hits both rubrics), Sea Glass Shard for Clover. The gift engine's match logic now folds `item_category` into the candidate set so Clover loves a `category="trinket"` smooth pebble without needing "trinket" in their casual likes list. `loved_tags` is intentionally absent from `public_villager_summary` so clients can't mine the rubric. New coverage in `server/tests/test_gift_relationship.py` (Fern/Hugo/Clover loved cases + the Hugo-on-Dusty-Rose neutral case) and `server/tests/test_personality_configs.py` (schema + signature tags). All 13 server tests green plus the live demo stack, websocket roundtrip, both Godot/web static tests, `node --check`, `compileall`, and `git diff --check` clean. `docs/AI_ARCHITECTURE.md`'s HH-006 status block now records HH-062 as shipped. The change is intentionally orthogonal to the persisted `mood_until` pin, first-of-kind gift bonus, and repeated-gift dampening — those remain on the recommended list for a later heartbeat.
...
- **2026-05-15 (overnight, twelfth heartbeat) - Codex (scheduled task):** Cleared the web-demo half of the HH-062 gift-picker follow-up. Before this heartbeat, `game/web/main.js` `renderGiftRow()` rendered each inventory item as a single-line button with no category cue and no hover hint, and `FALLBACK_INVENTORY` only mirrored the four pre-HH-062 starter gifts — so an offline demo would never show Lavender Sachet, Honey Oat Crust, Marigold Sprig, or Sea Glass Shard even though the bootstrap inventory does. Three changes fix that without touching the canonical `gift_item` WebSocket payload. (1) **Full offline catalog.** `FALLBACK_INVENTORY` now mirrors `server/world/inventory.py:STARTER_INVENTORY` byte-for-byte (`dusty_rose`, `chamomile_bundle`, `porcelain_button`, `smooth_pebble`, `lavender_sachet`, `honey_oat_crust`, `marigold_sprig`, `sea_glass_shard`) with matching `category`, `tags`, and `gift_prompt` fields, so an offline demo sees the full assortment. (2) **Richer button shape.** `renderGiftRow()` builds each button as a `<button class="gift-button" data-item-id="…" data-category="…">` with a `<span class="gift-name">` for the display name and a `<span class="gift-caption">` showing the category in lighter type. The button's `title` attribute (HTML tooltip) and `aria-label` are wired to the server's `gift_prompt`, so hovering a gift surfaces a one-line sensory hint ("A small heel of warm honey oat bread saved from this morning's bake") rather than only the name. (3) **CSS category stripes.** `index.html` gains `.gift-button` / `.gift-name` / `.gift-caption` rules plus category-keyed left-border stripes (`flower`/`herb` → sage, `baked` → clay, `keepsake`/`trinket` → blue), so Heather can scan the picker by gift shape rather than reading every name. The existing `renderChatChrome()` disable loop still uses `dom.giftRow.querySelectorAll("button")` and continues to disable the new buttons while a reply is pending. **Coverage.** `game/tests/test_web_demo_static.py` grew `test_gift_picker_surfaces_full_starter_inventory()` asserting (a) all eight canonical starter item ids appear in `main.js`, (b) the gift-button structure (`gift-button`/`gift-name`/`gift-caption` class names, `dataset.itemId`, `dataset.category`, the `button.title = promptHint` line, and the `item.category || "gift"` caption fallback), and (c) the CSS category selectors for flower/baked/trinket plus the name/caption rules. Added to `main()` so the runner picks it up. **Validation matrix (all green):** `python3 game/tests/test_web_demo_static.py`, `python3 game/tests/test_godot_project_static.py`, `node --check game/web/{main.js,scene.js}`, `python3 -m compileall server`, then the full 15-test `uv run --python 3.12 --with-requirements server/requirements.txt python -m server.tests.{test_memory_roundtrip,test_personality_configs,test_api_contract,test_mood,test_world_state,test_gift_relationship,test_conversation_relationship,test_social_memory_conversation,test_mobile_notifications,test_away_interactions,test_demo_seed,test_demo_storyline,test_ollama_provider,test_talk_caps,test_tuning_calibration}` matrix, plus the live stack `... test_live_demo_stack` and `... test_websocket_roundtrip --start-server` (both green; Margot's Dusty Rose reply still lands as `delighted`), and `git diff --check` clean. The Godot half of HH-062 (a dialogue-side gift picker popup so `_send_starter_gift()` doesn't always send the first item) is still open and remains in For Codex below — the web demo is the canonical morning-demo client, so this isn't blocking. Browser/runtime validation of the new button styling and tooltip behavior remains unrun in this heartbeat — static + JS-syntax checks are the durable proof.
...
- ~~**New follow-up (HH-062): show the new gift items in the client gift UI.**~~ Cleared in the 2026-05-15 Cowork overnight heartbeat (fourteenth). The **web demo gift picker** was already done in the twelfth Codex heartbeat (`renderGiftRow()` + `FALLBACK_INVENTORY` + CSS category stripes + `test_gift_picker_surfaces_full_starter_inventory()`). The **Godot gift picker** now matches: `game/scripts/main.gd` `_on_gift_pressed()` opens a `gift_picker_panel` PanelContainer (placed above the input row inside the dialogue stack) that rebuilds one button per `starter_inventory` entry with a category caption (`"%s — %s" % [display_name, caption]`) and a `gift_prompt` tooltip. Per-item sends go through a new `_send_gift_item(item)` helper that the picker shares with `_send_starter_gift()` (the keyboard `gift` shortcut still defaults to the Dusty Rose payload so muscle memory is preserved). `_close_dialogue()` also closes the picker. Coverage: `test_main_gift_picker_wiring()` in `game/tests/test_godot_project_static.py` asserts the picker scaffolding, gift-button entry-point text/tooltip, per-item helpers (`_gift_display_name` / `_gift_caption` / `_gift_tooltip` / `_gift_picker_items`), `_send_gift_item` / `_send_starter_gift` shape, picker-button binding via `connect(...).bind(bound_item)`, `_close_dialogue` -> `_close_gift_picker` lifecycle, and that the picker body never references private personality fields. Godot runtime validation is still blocked until a `godot4` binary is available; the static guardrail is the durable proof.
- ~~**New follow-up (HH-062 / HH-006 mood pin):** the loved-gift mood-tracker nudge weight 2.5 carries `delighted` for several ticks but is not a true pin. The recommended `pinned_mood` / `pinned_until_game_minute` schema on `mood_state` (see `docs/AI_ARCHITECTURE.md`) is still unshipped.~~ Cleared in the 2026-05-15 Cowork overnight heartbeat (ninth pass). `mood_state` now carries `pinned_mood` and `pinned_until_minute`. `MoodTracker.pin(...)` records the pin against a monotonic `(day - 1) * 1440 + minute_of_day` count, `tick()` reinforces and reports the pin while it's live then clears it on expiry, `nudge()` preserves the pin so a follow-up conversation doesn't wash it out, and `handle_gift` calls `pin()` only when `preference == "loved"`. Coverage: `run_mood_pin_check()` in `test_mood.py` plus per-villager loved-gift pin assertions in `test_gift_relationship.py`. The pin defaults to `DEFAULT_PIN_DURATION_MINUTES=120` (~2 in-game hours, the cozy "linger over a thoughtful gift" feel from `docs/AI_ARCHITECTURE.md`). Remaining HH-006 work: per-villager `tuning` JSON calibration (`loved_gift_mood_lock_hours`, `affection_per_talk_cap`, etc.), the first-of-kind gift bonus, and repeated-gift dampening. None blocking.
...
- ~~**HH-006 remaining: first-of-kind gift bonus and repeated-gift dampening.**~~ Cleared in the 2026-05-15 Cowork overnight heartbeat (tenth pass). `handle_gift` now bumps the very first gift any (player, villager) pair sees up one tier on `GIFT_PREFERENCE_LADDER` and halves the affection delta on the 3rd-or-later same-item gift within `GIFT_REPEAT_WINDOW_DAYS=3` in-game days. Persisted as `gift_first_done` (bool) and `recent_gifts: list[{item_id, day}]` on the player→villager relationship row, pruned at every write. Memory and event metadata gained `base_preference`, `first_of_kind_bonus`, `repeat_gift_dampened`, and `repeat_count_in_window` — none of which are in the public event/memory allowlists. Coverage: new `run_first_of_kind_gift_bonus_check()` and `run_repeated_gift_dampening_check()` in `server/tests/test_gift_relationship.py`. The existing steady-state Margot/Fern/Hugo/Clover assertions now run a `_burn_first_of_kind()` warmup gift per player so they exercise the native preference path. `docs/AI_ARCHITECTURE.md`'s HH-006 status block records both rules as shipped.
- ~~**New follow-up: per-villager `tuning` JSON calibration.**~~ Cleared in the 2026-05-15 Codex overnight heartbeat (eleventh pass). The four canonical MVP villager JSONs now ship calibrated tuning blocks (Margot slow-warming `affection_per_talk_cap=1`/`trust_per_talk_cap=0`/`loved_gift_mood_lock_hours=3`; Hugo gruff `affection_per_talk_cap=1`/`loved_gift_mood_lock_hours=3`; Clover testing-trust `trust_per_talk_cap=0`/`loved_gift_mood_lock_hours=1.5`; Fern intentionally untuned). Coverage: new `server/tests/test_tuning_calibration.py` plus extended `test_personality_configs.py` + `test_api_contract.py` bootstrap-leak guardrails. `docs/AI_ARCHITECTURE.md` HH-006 status block and the "Per-villager calibration" section record the field shape, supported keys, canonical-cast table, and validation pointers. The "Hugo's shared weather rainy-day greeting" affection bonus is a more nuanced event-hook hook that this heartbeat does *not* cover — it'd require classifying conversation context against world weather and is left as a future heartbeat if the demo wants the extra texture.
- ~~**New follow-up: Hugo shared-weather and Clover day-5 trust unlock event hooks.**~~ Cleared in the 2026-05-15 Codex overnight heartbeat (thirteenth pass). Both shipped exactly as the prior heartbeat suggested — as new optional `tuning` keys so the rubric stays in JSON. `Personality._coerce_tuning` now accepts `trust_cap_unlocks_on_day` (int > 0) and `shared_weather_affection_bonus` (int ≥ 0); `_effective_talk_caps` drops a `trust_per_talk_cap` override when `world_day >= first_seen_day + N`; a new `_shared_weather_bonus()` helper in `conversation.py` adds a +N affection delta on top of the capped talk delta when the player text mentions the world's current weather (rain/storm/snow/sun/cloud vocabulary against the world `weather` substring) or the literal time-of-day word against the world `time_label`. The bonus stacks past the daily cap so the moment lands but only fires once per in-game day per (player, villager) via `last_shared_weather_day`. `first_seen_day` is now stamped on the very first interaction (talk *or* gift) and is never overwritten, so the trust unlock has a stable reference point. Clover JSON gains `trust_cap_unlocks_on_day=5`; Hugo JSON gains `shared_weather_affection_bonus=1`. Coverage: five new runners in `server/tests/test_tuning_calibration.py` (`run_clover_trust_unlock_check`, `run_clover_trust_unlock_locked_window_check`, `run_hugo_shared_weather_bonus_check`, `run_hugo_shared_evening_bonus_check`, `run_fern_no_shared_weather_bonus_check`); `test_personality_configs.py` validates the two new tuning keys + locks the canonical-cast values; `test_api_contract.py`'s tuning-leak guardrail still catches any accidental public exposure (the bootstrap allowlist is unchanged). `docs/AI_ARCHITECTURE.md`'s HH-006 status block, supported-keys list, per-villager bullets, and the canonical-cast calibration table all record the change.
...
Latest Cowork heartbeat check (fourteenth — scheduled-task pass): **2026-05-15 (overnight)**. Cleared the last open client-side HH-062 item — **the Godot dialogue-side gift picker**. Before this heartbeat, `game/scripts/main.gd` `_on_gift_pressed()` always called `_send_starter_gift()`, which sent the cached Dusty Rose payload — even though `/client/bootstrap` was already populating `starter_inventory` with all eight HH-062 starter items. The web demo's twelfth-heartbeat picker is now mirrored in the Godot panel: a new `gift_picker_panel: PanelContainer` + `gift_picker_list: VBoxContainer` is built inside `_build_ui()` and slotted into the dialogue stack directly above the input row. When `gift_button` is pressed, `_open_gift_picker()` rebuilds the list from the cached `starter_inventory` (falling back to `[_starter_gift_payload()]` so the picker is never empty), and each item becomes a button labeled `"display_name — category"` with the server's `gift_prompt` as the hover tooltip. Pressing a picker button calls `_on_gift_picker_item_pressed(item)` -> `_send_gift_item(item)`, which routes through the same `_interaction_context(villager, {"gift_source": "starter_inventory"})` payload the old code path used — only the gift dict changes. The keyboard `gift` shortcut still defaults to the Dusty Rose via `_send_gift_item(_starter_gift_payload())` so muscle memory is preserved. `_close_dialogue()` calls `_close_gift_picker()` so the list doesn't outlive the dialogue. `_apply_bootstrap_inventory()` now updates the gift-button tooltip to "Choose a gift from the starter inventory (N items)." instead of advertising a specific item. **Coverage:** new `test_main_gift_picker_wiring()` runner in `game/tests/test_godot_project_static.py` asserts the picker scaffolding (`gift_picker_panel: PanelContainer`, `gift_picker_list: VBoxContainer`, `_gift_picker_panel_style()` StyleBoxFlat), the gift-button entry-point behavior including the negative assertion that the old `"Gift Rose"` text is gone, every new per-item helper (`_gift_display_name` / `_gift_caption` / `_gift_tooltip` / `_gift_picker_items` / `_normalize_gift_item`), the `_send_gift_item` / `_send_starter_gift` split, the picker-button signal binding via `connect(...).bind(bound_item)`, the `_close_dialogue` -> `_close_gift_picker` lifecycle, and that the picker body never references private personality fields. **Validation matrix (all green):** `python3 game/tests/test_godot_project_static.py`, `python3 game/tests/test_web_demo_static.py`, `node --check game/web/{main.js,scene.js}`, `python3 -m compileall server`, plus the full 15-test `uv run --python 3.12 --with-requirements server/requirements.txt python -m server.tests.{test_memory_roundtrip,test_personality_configs,test_api_contract,test_mood,test_world_state,test_gift_relationship,test_conversation_relationship,test_social_memory_conversation,test_mobile_notifications,test_away_interactions,test_demo_seed,test_demo_storyline,test_ollama_provider,test_talk_caps,test_tuning_calibration}` matrix, the live stack `... test_live_demo_stack` and `... test_websocket_roundtrip --start-server` (both green; Margot's Dusty Rose reply still lands as `delighted`), and `git diff --check` clean. The stale `.git/index.lock` re-appeared mid-heartbeat as in earlier overnight runs; cleared via the Desktop Commander host shell per the documented protocol. **Demo impact:** the Godot client now reads the same HH-062 per-villager loved-gift rubric as the web demo — Heather can pick Lavender Sachet for Fern, Honey Oat Crust for Hugo, Marigold Sprig or Sea Glass Shard for Clover, and the canonical Dusty Rose for Margot, with each one landing as a `delighted` reply on the correct villager. Godot runtime validation remains blocked until a `godot4` binary is available; the static guardrail is the durable proof.

Earlier Codex heartbeat check (thirteenth — scheduled-task pass): **2026-05-15 (overnight)**. Cleared the last unstruck HH-006 doc-level follow-up: **Hugo shared-weather and Clover day-5 trust unlock event hooks**, both landed as new optional `tuning` keys so the rubric stays in JSON. `Personality._coerce_tuning` now accepts `trust_cap_unlocks_on_day` (int > 0) and `shared_weather_affection_bonus` (int ≥ 0). `_effective_talk_caps` reads the persisted `first_seen_day` off the relationship row and drops a `trust_per_talk_cap` override once `world_day >= first_seen_day + N`. A new `_shared_weather_bonus()` helper in `server/ai/conversation.py` reads the player text against a small weather/time vocabulary (rain/storm/snow/sun/cloud + morning/afternoon/evening/night) and matches it against the world snapshot's `weather` substring / `time_label`; on a match it returns the personality's `shared_weather_affection_bonus` int plus a marker like `weather:rain` or `time:evening`. `handle_player_message` adds the bonus on top of the capped talk delta (so it actually lands past Hugo's tight `+1` cap), stamps `last_shared_weather_day` and `last_shared_weather_marker` on the relationship row to enforce the once-per-in-game-day fire, and stamps `first_seen_day` on the very first interaction. `handle_gift` also stamps `first_seen_day` on its first run for parity (a gift counts as meeting Clover). All four metadata fields are server-private — they're inputs to the tuning logic, not stat-sheet readings for clients, and the existing `test_api_contract.py` tuning-leak guardrails still hold. **Per-villager wiring:** Clover's JSON gains `trust_cap_unlocks_on_day=5` (her `trust_per_talk_cap=0` override now expires five in-game days after first meeting and the global default reasserts); Hugo's JSON gains `shared_weather_affection_bonus=1` (a "thanks Hugo, I love this rain" on a rainy day lands him +2 affection in one turn — the +1 capped talk plus the +1 shared-weather bonus). **Coverage:** five new runners in `server/tests/test_tuning_calibration.py` — `run_clover_trust_unlock_check` (day-1 turn caps trust at 0; world advances five in-game days; day-6 turn lands trust=1 and `first_seen_day` is preserved), `run_clover_trust_unlock_locked_window_check` (day-5 turn is still locked because the unlock condition is strict `>=`), `run_hugo_shared_weather_bonus_check` (rainy day stacks +2 affection past the +1 cap; same-day repeat does not refire; new in-game day allows another fire; clear weather suppresses the bonus and skips the metadata write), `run_hugo_shared_evening_bonus_check` (evening time_label + "good evening" lands the bonus with marker `time:evening`), and `run_fern_no_shared_weather_bonus_check` (a villager without the tuning key never sees the bonus and never writes the marker fields). `test_personality_configs.py` validates the two new tuning keys (schema check + canonical-cast lock-in for Hugo/Clover). **Validation matrix (all green):** the full 15-server-test `uv run --python 3.12 --with-requirements server/requirements.txt python -m server.tests.{test_memory_roundtrip,test_personality_configs,test_api_contract,test_mood,test_world_state,test_gift_relationship,test_conversation_relationship,test_social_memory_conversation,test_mobile_notifications,test_away_interactions,test_demo_seed,test_demo_storyline,test_ollama_provider,test_talk_caps,test_tuning_calibration}` matrix + `... test_live_demo_stack` + `... test_websocket_roundtrip --start-server` (Margot's "Oh, Dusty Rose..." reply still lands as `delighted`) + `python3 game/tests/test_godot_project_static.py` + `python3 game/tests/test_web_demo_static.py` + `node --check game/web/{main.js,scene.js}` + `uv run --python 3.12 --with-requirements server/requirements.txt python -m compileall server` + `git diff --check` clean. **Demo impact:** Hugo's bonus is the more visible piece for the morning demo because the world weather is set in code (default `clear`) and any Sterling-flipped `engine.world_state.weather = "rainy"` or evening time_label would let Heather feel a deliberate +affection moment when she greets Hugo with the right line. Clover's day-5 unlock is a longer-arc reward and won't show inside a typical demo session unless `HOLLOW_DAY_LENGTH_SECONDS` is set very low and the session runs long enough to cross five in-game days — but mechanically it's now correct and tested. With this heartbeat, the only HH-006 work remaining is the cosmetic "+1 less than the baseline for the first three visits" half of Hugo's rubric, which is implicitly approximated by his `affection_per_talk_cap=1` already; making it strict-three-visits would need a separate `visit_count` field on the relationship row.

Earlier Codex heartbeat check (twelfth — scheduled-task pass): **2026-05-15 (overnight)**. Cleared the web-demo half of the HH-062 gift-picker follow-up. `game/web/main.js` `FALLBACK_INVENTORY` now mirrors all eight `server/world/inventory.py:STARTER_INVENTORY` items (including Lavender Sachet / Honey Oat Crust / Marigold Sprig / Sea Glass Shard), and `renderGiftRow()` builds each button as a category-tinted `gift-button` with a name span, a small lowercase category caption, and a hover tooltip / aria-label sourced from the server's `gift_prompt`. `index.html` gains `.gift-button` / `.gift-name` / `.gift-caption` rules plus category-keyed left-border stripes (flower/herb → sage, baked → clay, keepsake/trinket → blue) so the picker is scannable by gift shape. The canonical `gift_item` WebSocket payload, the `renderChatChrome()` disable loop, and the existing `sendGift()` flow are unchanged. **Coverage:** new `test_gift_picker_surfaces_full_starter_inventory()` runner in `game/tests/test_web_demo_static.py` asserts all eight item ids, the button structure, the tooltip wiring, and the CSS category selectors. **Validation matrix (all green):** `python3 game/tests/test_web_demo_static.py`, `python3 game/tests/test_godot_project_static.py`, `node --check game/web/{main.js,scene.js}`, `python3 -m compileall server`, plus the full 15-test `uv run --python 3.12 --with-requirements server/requirements.txt python -m server.tests.{test_memory_roundtrip,test_personality_configs,test_api_contract,test_mood,test_world_state,test_gift_relationship,test_conversation_relationship,test_social_memory_conversation,test_mobile_notifications,test_away_interactions,test_demo_seed,test_demo_storyline,test_ollama_provider,test_talk_caps,test_tuning_calibration}` matrix, the live stack `... test_live_demo_stack` and `... test_websocket_roundtrip --start-server` (both green; Margot's Dusty Rose reply still lands as `delighted`), and `git diff --check` clean. The Godot half of HH-062 (dialogue-side gift picker popup so `_send_starter_gift()` doesn't always pull `starter_inventory[0]`) is still open in For Codex — the web demo is the canonical morning-demo client, so this isn't blocking. Browser/runtime validation of the new button styling and tooltip remains unrun (static + JS-syntax checks are the durable proof).

Earlier Codex heartbeat check (eleventh — scheduled-task pass): **2026-05-15 (overnight)**. Shipped the last unstruck HH-006 follow-up: per-villager `tuning` JSON calibration. `Personality` now accepts an optional `tuning: dict[str, Any]` block whose five supported keys (`affection_per_talk_cap`, `trust_per_talk_cap`, `negative_talk_per_day_cap`, `loved_gift_mood_lock_hours`, `first_gift_bonus_tier`) are coerced to their expected types at load time and override the global HH-006 constants on a per-villager basis. `_apply_talk_caps` (now takes an optional `personality`) reads the talk caps; `handle_gift` reads the loved-gift pin duration and the first-of-kind opt-out. Canonical MVP cast JSONs now ship calibrated tuning: Margot (fragile/slow-warming/loved-gifts-feel-biggest), Hugo (gruff/slow-trust/lingers-on-praise), Clover (testing/trust-is-earned/moves-on-quickly); Fern intentionally untuned because the defaults *are* her register. `tuning` is server-private — not in `public_villager_summary`, bootstrap, or villager-context payloads. Coverage: new `server/tests/test_tuning_calibration.py` (six runners), extended `test_personality_configs.py` (schema + calibration lock-in) and `test_api_contract.py` (leak guardrails); rerouted `test_talk_caps.py` from Margot to Fern so it still exercises the *default* cap path; updated `test_conversation_relationship.py` for Margot's new `trust_per_talk_cap=0` on a personal-disclosure turn. **Validation matrix (all green):** full 15-server-test `uv run --python 3.12 --with-requirements server/requirements.txt python -m server.tests.{test_memory_roundtrip,test_personality_configs,test_api_contract,test_mood,test_world_state,test_gift_relationship,test_conversation_relationship,test_social_memory_conversation,test_mobile_notifications,test_away_interactions,test_demo_seed,test_demo_storyline,test_ollama_provider,test_talk_caps,test_tuning_calibration}` + `test_live_demo_stack` + `test_websocket_roundtrip --start-server` (Margot's "Oh, Dusty Rose..." reply still lands as `delighted`) + `python3 game/tests/test_godot_project_static.py` + `python3 game/tests/test_web_demo_static.py` + `node --check game/web/{main.js,scene.js}` + `python3 -m compileall server` + `git diff --check`. Godot runtime validation remains blocked until a Godot 4 binary is available; browser runtime validation also remains unrun in this heartbeat (static + JS-syntax checks are the durable proof). The only HH-006 work remaining is the more nuanced event-hook follow-ups (Hugo's shared-weather affection bump, Clover's day-5 trust unlock); those need new event-detection scaffolding rather than another JSON knob and are not blocking the morning demo.

Earlier Codex heartbeat check: **2026-05-15 08:08 MDT / 2026-05-15 14:08 UTC**. Completed HH-059 root browser demo promotion and archived the stale root `codex-demo.html` into `docs/prototypes/`. Next Codex priority is HH-061 (web visual/runtime polish) unless Sterling redirects to HH-004 multi-villager Godot work. Godot runtime validation remains blocked until a Godot 4 binary is available. Browser runtime validation also remains unrun in this heartbeat; static protocol checks and JS syntax checks passed.

Latest Cowork heartbeat: **2026-05-15 (overnight, tenth — scheduled-task pass)**. Shipped the last two HH-006 gift-path follow-ups that the ninth heartbeat had left on the recommended list — **first-of-kind gift bonus** and **repeated-gift dampening**. `server/ai/conversation.py:handle_gift` now bumps the very first gift each (player, villager) pair sees up one tier on `GIFT_PREFERENCE_LADDER = ("disliked","neutral","liked","loved")` and halves the affection delta toward zero on the 3rd-or-later same-`item_id` gift within `GIFT_REPEAT_WINDOW_DAYS=3` in-game days. Persisted as `gift_first_done` and a pruned `recent_gifts: list[{item_id, day}]` on the player→villager relationship row. Trust deltas, familiarity deltas, mood selection, the loved-gift `MoodTracker.pin()`, the memory write, and the event publish are intentionally left alone by both rules — only the affection delta and the public-mood label change. Memory + event metadata now record `base_preference`, `first_of_kind_bonus`, `repeat_gift_dampened`, and `repeat_count_in_window` (none in the public allowlists). New coverage: `run_first_of_kind_gift_bonus_check()` and `run_repeated_gift_dampening_check()` in `server/tests/test_gift_relationship.py`, plus a `_burn_first_of_kind()` warmup gift threaded through every existing steady-state assertion so the legacy cases still test the native preference path. Validation matrix (all green): full 14-server-test matrix + `test_live_demo_stack` + `test_websocket_roundtrip --start-server` + `test_godot_project_static.py` + `test_web_demo_static.py` + `node --check game/web/{main.js,scene.js}` + `python -m compileall server` + `git diff --check`. `docs/AI_ARCHITECTURE.md`'s HH-006 status block and the "Recommended deltas" table now list both rules as shipped. Remaining HH-006 work after this heartbeat is the optional per-villager `tuning` JSON (e.g. Margot's slow-warming `affection_per_talk_cap=1`, Hugo's rainy-day affection bonus, Clover's day-5 trust cap unlock). None blocking the morning demo. See the tenth-heartbeat note in Completed for the change details.

Earlier Cowork heartbeat: **2026-05-15 (overnight, ninth — scheduled-task pass)**. Shipped the HH-006 mood-pin schema that prior heartbeats had left on the recommended-but-unshipped list. `mood_state` now carries optional `pinned_mood: str | None` and `pinned_until_minute: int | None` fields. A new `MoodTracker.pin(villager_id, mood, *, world, duration_minutes=120)` records the pin against a monotonic in-game-minute count `(day - 1) * 1440 + minute_of_day` computed from the world snapshot. `tick()` is now pin-aware: while the pin is active it adds a `PINNED_MOOD_TICK_REINFORCEMENT=0.45` weight on every baseline tick (so score-resolved dominant doesn't fight the pin) and reports the pinned mood regardless of baseline; when the deadline elapses it clears the pin metadata and lets the baseline reassert. `nudge()` preserves an existing pin's metadata and forces the resolved current to the pinned mood so a conversation turn during the pin window doesn't wash it out. `current_mood(..., world=...)` returns the pinned mood when world context is provided and the pin is live, falling back to the persisted `current` when world is absent so existing callers keep working unchanged. `handle_gift` in `server/ai/conversation.py` calls `mood_tracker.pin(villager_id, tracker_mood, world=self.world_state)` only when `preference == "loved"` — liked/neutral/disliked gifts intentionally don't pin, matching the cozy "loved gifts feel biggest, missteps are a small note" rubric. The public `/villagers/{id}` and `/client/villagers/{id}/context` payloads surface the pinned reading via the existing `mood_state.current` write — the `pinned_mood`/`pinned_until_minute` fields themselves stay server-private (no API contract change needed). Coverage: new `run_mood_pin_check()` in `test_mood.py` pins `excited` on Margot at day-1 10:00, runs several baseline ticks inside the window (asserting current stays excited), then ticks past the 120-minute deadline (asserting pin metadata clears); per-villager loved-gift pin assertions in `test_gift_relationship.py` for Fern/Hugo/Clover plus a guard that the disliked path doesn't pin. Full validation matrix green (14 server tests + live demo stack + websocket roundtrip + both Godot/web static tests + node syntax + compileall + git diff --check). `docs/AI_ARCHITECTURE.md`'s HH-006 status block now records the pin as shipped. Remaining HH-006 follow-ups: per-villager `tuning` JSON calibration (e.g. Margot's slower-trust personality, Fern's "you remembered" bonus), the first-of-kind gift bonus, and repeated-gift dampening (Dusty-Rose-spam → halved affection delta from the third same-item gift in 3 days). None blocking for the morning demo. See the ninth-heartbeat note in Completed for the change details.

Earlier Cowork heartbeat: **2026-05-15 (overnight, eighth — scheduled-task pass)**. Shipped HH-062 per-villager loved-gift rubric. Each canonical MVP villager now has a JSON-driven `loved_tags` list so the gift engine reads as personality rather than a uniform rubric — Margot still loves a Dusty Rose, Fern loves a Lavender Sachet, Hugo loves a Honey Oat Crust, Clover loves a Marigold Sprig or Sea Glass Shard, and Hugo specifically does *not* love Dusty Rose (the cast-feel regression that motivated the whole change). Four new starter inventory items, full schema + signature-tag coverage in `test_personality_configs.py`, three new loved-gift cases + a Hugo-on-Dusty-Rose neutral case in `test_gift_relationship.py`. Full validation matrix green (13 server tests + live demo stack + websocket roundtrip + both Godot/web static tests + node syntax + compileall + git diff --check). The mood-pin schema, first-of-kind gift bonus, and repeated-gift dampening remain on the recommended-but-unshipped list. See the eighth-heartbeat note in Completed for the change details.

Earlier Cowork heartbeat: **2026-05-15 (overnight, seventh — scheduled-task pass)**. Pivoted off web polish to land the HH-006 talk-path tuning the prior heartbeats had left in the For Codex list. Talk now respects per-in-game-day caps (+2 affection, +1 trust, single -1 with a 0 floor for talk alone), personal disclosure earns a heavier mood nudge (0.6 vs 0.45), disliked gifts use a softer mood nudge (0.35 vs 0.55), and `irritated`/`anxious` get clamped to 0.7 on every mood write so villagers can't stew. New `server/tests/test_talk_caps.py` plus the extended `test_mood.py` cover the new behavior, and `docs/AI_ARCHITECTURE.md`'s HH-006 status now reflects what shipped vs what's still recommended. Full validation matrix green. See the seventh-heartbeat note in Completed for the change details and the remaining HH-006 follow-ups.
