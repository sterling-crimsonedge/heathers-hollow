...
- **New follow-up: per-villager `tuning` JSON calibration.** With the talk caps, gift softening, loved-gift pin, per-villager loved rubric, first-of-kind bonus, and repeated-gift dampening all shipped, the last unstruck HH-006 item is the optional per-villager `tuning` object proposed in `docs/AI_ARCHITECTURE.md` (e.g. Margot getting a lower `affection_per_talk_cap`, Hugo getting a "shared weather" affection bonus on rainy-day greetings, Clover's trust cap halved for the first 5 in-game days). None of this is blocking the morning demo; the global defaults are already cozy.
