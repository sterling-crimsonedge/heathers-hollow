...
- **New follow-up (HH-062 / HH-006 mood pin):** the loved-gift mood-tracker nudge weight 2.5 carries `delighted` for several ticks but is not a true pin. The recommended `pinned_mood` / `pinned_until_game_minute` schema on `mood_state` (see `docs/AI_ARCHITECTURE.md`) is still unshipped. If the morning demo shows `delighted` getting overwritten by the next baseline tick faster than feels right, picking that up next is the smallest mechanical change with the biggest demo-feel payoff. None of this is blocking.
