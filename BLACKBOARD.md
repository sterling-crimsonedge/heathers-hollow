...
- ~~**Optional HH-006 follow-up implementation.**~~ Gift-path tuning done. The remaining piece is a *true* mood pin (`pinned_mood`/`pinned_until_game_minute` on `mood_state`) so loved gifts hold `excited` for ~2 in-game hours regardless of how many ticks fire. Talk-path tuning (positive/personal/negative/neutral deltas) from `docs/AI_ARCHITECTURE.md` is also still open.
...
- **New follow-up: draw a brook landmark in `game/web/scene.js`.** This heartbeat added `brook` to `LOCATION_POINTS` so Clover has a distinct (4, 5) spot in the canvas village, but there's no `drawBrook()` like `drawGarden()`/`drawFountain()` yet. A small blue meander between Margot's square and Hugo's shop, with a couple of marigold sprites on the bank, would visually anchor Clover's home and make the cast-doc chipped-saucer beat readable. Pure presentation work; no server changes needed.
- **New follow-up: HH-004 multi-villager spawn in Godot.** Concretely: in `game/scripts/main.gd`, change the single Margot spawn into a loop over the bootstrap `villagers` array (already cached on startup), instantiate one `villager.tscn` per entry, set `home_location` from each entry, and place at a `Vector3` looked up from a small in-script map (`town_square` → existing Margot position; `garden`, `shop`, and `brook` → new positions roughly matching the web demo's `LOCATION_POINTS`). Wire proximity-based talk/gift targeting against `_nearest_villager()`. Extend `test_godot_project_static.py` to assert the multi-villager wiring (e.g. a `_spawn_villagers_from_bootstrap` function exists and references each villager id) since runtime validation stays blocked.
