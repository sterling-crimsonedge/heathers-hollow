...
- **New follow-up (HH-062): show the new gift items in the client gift UI.** *Partially cleared in the twelfth Codex heartbeat (2026-05-15 overnight).* The **web demo gift picker is done** — `game/web/main.js` `renderGiftRow()` now renders a category-tinted button per inventory item with a `gift_prompt` tooltip and a small category caption; `FALLBACK_INVENTORY` mirrors all eight `STARTER_INVENTORY` items so offline mode also shows every gift; CSS category stripes (sage for flower/herb, clay for baked, blue for keepsake/trinket) make the picker scannable; `test_gift_picker_surfaces_full_starter_inventory()` in `game/tests/test_web_demo_static.py` locks in the eight item ids, the button stack, the tooltip wiring, and the CSS category selectors so the picker can't silently regress. The **Godot gift picker is still open** — `game/scripts/main.gd` `_send_starter_gift()` still pulls `starter_inventory[0]` (Dusty Rose) every time. Wiring a Godot picker needs a dialogue-side popup that lets Heather select an item from `starter_inventory` before sending, plus matching static coverage in `test_godot_project_static.py`. None blocking the morning demo since the web demo is the canonical client for the cozy showcase.
