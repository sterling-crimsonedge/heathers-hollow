...
- **Hugo** is slow-trust, slow-fall. His affection should move +1 less than the baseline for the first three visits, then move +1 more than the baseline after a "shared weather" line (rainy day greetings, evening greetings). This is the cozy version of "earning his respect." *(The "first three visits" half is approximated by Hugo's `affection_per_talk_cap=1`. The "shared weather" half shipped in the twelfth Codex heartbeat as `shared_weather_affection_bonus=1` — see the supported-keys table below for behaviour and the canonical-cast calibration row.)*
- **Clover** moves familiarity faster and trust slower. Cap their trust at half the others' caps for the first 5 in-game days, then unlock — Clover is testing whether Heather keeps her promises. *(Shipped twelfth Codex heartbeat as `trust_per_talk_cap=0` plus `trust_cap_unlocks_on_day=5`; the unlock fires when `world_day >= first_seen_day + 5`. `first_seen_day` is stamped on the relationship row's metadata on the very first interaction.)*

These per-villager modifiers should live in personality JSON, not in branching `if villager_id == "margot"` code. Field shape (shipped, 2026-05-15 Codex overnight heartbeat):

```json
"tuning": {
  "affection_per_talk_cap": 1,
  "trust_per_talk_cap": 0,
  "negative_talk_per_day_cap": 1,
  "loved_gift_mood_lock_hours": 3,
  "first_gift_bonus_tier": 1
}
```

Supported keys (others are dropped at load time by `Personality._coerce_tuning`):

- `affection_per_talk_cap` (int ≥ 0): overrides `TALK_AFFECTION_DAILY_CAP` for this villager.
- `trust_per_talk_cap` (int ≥ 0): overrides `TALK_TRUST_DAILY_CAP`.
- `negative_talk_per_day_cap` (int ≥ 0): overrides `TALK_NEGATIVE_DAILY_CAP`.
- `loved_gift_mood_lock_hours` (float > 0): overrides the default ~2-hour `MoodTracker.pin` window on a loved gift. Internally converted to in-game minutes (`hours × 60`).
- `first_gift_bonus_tier` (int): set to `0` to disable the HH-006 first-of-kind tier bump for this villager. Default `1` (enabled).
- `trust_cap_unlocks_on_day` (int > 0): days after first meeting after which the per-villager `trust_per_talk_cap` override is dropped and the global trust cap reasserts. Models the cozy "they were testing whether Heather keeps promises, and now they trust her" arc. The unlock fires when `world_day >= first_seen_day + trust_cap_unlocks_on_day`; `first_seen_day` is stamped on the relationship row's metadata on the very first interaction (talk *or* gift) and is never overwritten. Currently used by Clover (`5`).
- `shared_weather_affection_bonus` (int ≥ 0): extra affection delta added when the player's text mentions the world's current weather (e.g. "rain" while `weather` contains "rain") or time-of-day (the literal word `morning`/`afternoon`/`evening`/`night` while `time_label` matches). The bonus stacks on top of the capped talk delta and bypasses the daily affection cap so the moment actually lands, but only fires once per in-game day per (player, villager) pair (tracked via `last_shared_weather_day` on the relationship metadata). Currently used by Hugo (`1`).

Canonical-cast calibration (shipped in `server/data/personalities/*.json`):

| Villager | `affection_per_talk_cap` | `trust_per_talk_cap` | `loved_gift_mood_lock_hours` | Event hooks | Rationale |
| --- | --- | --- | --- | --- | --- |
| Margot | 1 | 0 | 3 | — | Fragile and slow-warming. Loved gifts feel biggest, disliked gifts feel small. |
| Fern | (default 2) | (default 1) | (default ~2) | — | Rewards consistency over big gestures; the defaults are her register. |
| Hugo | 1 | (default 1) | 3 | `shared_weather_affection_bonus=1` | Slow-trust, slow-fall. Loved gifts linger because praise sticks with him. He warms when Heather notices the rain or the evening. |
| Clover | (default 2) | 0 | 1.5 | `trust_cap_unlocks_on_day=5` | Trust is *earned* through kept promises. Their override caps trust at 0 until five in-game days after first meeting, then the global default reasserts. |

Defaults match the table above so omitting `tuning` keeps current behaviour. Per-villager assertions live in `server/tests/test_personality_configs.py` (calibration lock-in) and `server/tests/test_tuning_calibration.py` (mechanical behaviour: caps fire, pin duration changes, first-of-kind opt-out works, day-5 trust unlock fires only after the eligible in-game day, shared-weather bonus fires once per day on matching world weather/time and not at all without the override). `tuning` is *not* exposed in `public_villager_summary`, `/villagers/{id}`, or `/client/bootstrap` — verified in `server/tests/test_api_contract.py`. The `first_seen_day`, `last_shared_weather_day`, and `last_shared_weather_marker` metadata fields are stamped on the player→villager relationship row by `handle_player_message`/`handle_gift` and are intentionally server-private (they're inputs to the tuning logic, not stat-sheet readings for clients).
