...
| Villager | `affection_per_talk_cap` | `trust_per_talk_cap` | `loved_gift_mood_lock_hours` | Rationale |
| --- | --- | --- | --- | --- |
...
| Hugo | 1 | (default 1) | 3 | Slow-trust, slow-fall. Loved gifts linger because praise sticks with him. |
| Clover | (default 2) | 0 | 1.5 | Trust is *earned* through kept promises (event-hook future work), not free from talk. Clover moves on quickly. |

Defaults match the table above so omitting `tuning` keeps current behaviour. Per-villager assertions live in `server/tests/test_personality_configs.py` (calibration lock-in) and `server/tests/test_tuning_calibration.py` (mechanical behaviour: caps fire, pin duration changes, first-of-kind opt-out works). `tuning` is *not* exposed in `public_villager_summary`, `/villagers/{id}`, or `/client/bootstrap` — verified in `server/tests/test_api_contract.py`.
